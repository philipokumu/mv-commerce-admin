  ...
  <div class="py-4 px-4 md:px-8">
    ...
    <div v-else>
            ...
            <tbody class="bg-white divide-y">
              <tr v-for="(seller, index) in sellers" :key="index">
                <td class="px-6 py-4 whitespace-nowrap">
                  <p>{{ seller.data.attributes.seller_name }}</p>
                  <p class="text-sm text-gray-500">
                    {{ seller.data.attributes.preserve_email }}
                  </p>
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap">
                  <p>{{ seller.data.attributes.seller_phone }}</p>
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap">
                  <p
                    ...
                    :class="{
                      'text-green-600 bg-green-200':
                        seller.data.attributes.active === 'active',
                      'text-red-600 bg-red-200':
                        seller.data.attributes.active === 'inactive'
                    }"
                  >
                    {{ seller.data.attributes.active }}
                  </p>
                </td>
                <td class="px-6 py-4 text-sm whitespace-nowrap text-right">
                  <nuxt-link
                    :to="`/sellers/${seller.data.seller_id}?tab=products`"
                    class="ml-2 inline-block text-green-500 hover:text-green-600 focus:text-green-600 focus:outline-none"
                  >
                    View
                  </nuxt-link>
                  <nuxt-link
                    :to="`/sellers/${seller.data.seller_id}/edit`"
                  ...
                  <button
                    ...
                    :disabled="busy"
                    @click="deleteSeller(seller.data.seller_id)"
                  >
                    Delete
                  </button>
                </td>
      ...
      <div v-else class="flex flex-col items-center justify-center">
        <div class="mb-4">
          <div class="mb-2 mx-auto h-24 w-24 text-gray-300"><shop-icon /></div>
